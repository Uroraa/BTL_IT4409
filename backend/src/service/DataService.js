...
const getAlerts = async (req, res) => {
  try {
    ...
    const fetchLimit = 240;
    const data = await Data.find({}).sort({ _id: -1 }).limit(fetchLimit);

    const thresholds = {
      temp: { threshold: 26, dir: 'high', name: 'Nhiệt độ' },
      humi: { threshold: 45, dir: 'low', name: 'Độ ẩm' },
      light: { threshold: 60, dir: 'high', name: 'Ánh sáng' }
    };
    ...
    data.forEach(pt => {
      ['temp', 'humi', 'light'].forEach(sensorKey => {
        ...
        if (lvl !== 'normal') {
          alerts.push({
            time: formatTimeDisplay(pt.time),
            sensorKey,
